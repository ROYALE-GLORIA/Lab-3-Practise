...
# File Created: Wednesday, 4th February 2026 8:06:39 PM
# Author: #TODO: Add author name
# Email: gloria.otieno@ashesi.edu.gh
# Version: 1.0.0
# Brief: <<brief>>
# -----
# Last Modified: Wednesday, 4th February 2026 8:06:39 PM
...
if [[ -z "${USER_HOME}" ]]; then
    readonly USER_HOME="${HOME:-/home/ros2_ws}"
fi
if [[ -z "${DEV_DIR}" ]]; then
...
setup_ros() {
    ...
    local ros_setup_file="/opt/ros/$ros2_distro/setup.$SHELL_TYPE"
    if [[ ! -f "$ros_setup_file" ]]; then
        log_error "ROS2 $ros2_distro not found. Please install ROS2 $ros2_distro"
        return 1
    fi
